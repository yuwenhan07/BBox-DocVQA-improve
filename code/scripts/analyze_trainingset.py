...
DEFAULT_REFERENCE_PNG = Path(
    ...
    "BBox-DocVQA-improve/training/cs/2303.13740/1.png"
)
...
def analyze_dataset(input_path, reference_png_path):
    ...
    reference_width, reference_height = png_size(reference_png_path)
    ...
    with input_path.open("r", encoding="utf-8") as infile:
        for line in infile:
            ...
            modalities = []
            flatten_strings(
                record.get("subimg_type", record.get("subimg_tpye", [])),
                modalities,
            )
            ...
            for page_idx, _page in enumerate(pages):
                ...
                for box in page_boxes:
                    ...
                    area_ratio = (box_width * box_height) / (reference_width * reference_height)
                    bbox_area_ratios.append(area_ratio)
                    center_x = min(max((x1 + x2) / 2 / reference_width, 0.0), 0.999999)
                    center_y = min(max((y1 + y2) / 2 / reference_height, 0.0), 0.999999)
...
def main():
    ...
    parser.add_argument(
        "--reference-png",
        default=str(DEFAULT_REFERENCE_PNG),
        help="Reference PNG used for all bbox size and position normalization.",
    )
    parser.add_argument(
        "--output-dir",
    ...
    reference_png_path = Path(args.reference_png).resolve()
    output_dir = Path(args.output_dir).resolve()
    ensure_dir(output_dir)

    summary, tables, series = analyze_dataset(input_path, reference_png_path)
